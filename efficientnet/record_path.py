...
parser.add_argument('--dataroot',type=str,default='/eva_data/zchin/rsna_data_all',help='raw data dir')
...
if __name__=='__main__':
    ...
    for annot_f in os.listdir(annot_dir):
        ...
        img_path=os.path.join(args.dataroot,'images/train',annot_f).replace('txt','png')
    ...
    for annot_f in os.listdir(annot_dir):
        ...
        img_path=os.path.join(args.dataroot,'images/val',annot_f).replace('txt','png')
        f.write(f'{img_path},{label}')
        f.write('\n')
    f.close()
